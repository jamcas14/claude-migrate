...
def load_profile(name: str) -> Profile:
    ...
    if raw is None:
        ...
        if name in blob:
            ...
            raise AuthMissing(
                f"No profile named {name!r} found. Run `claude-migrate add {name}` first."
            )
    else:
        try:
            data = json.loads(raw)
            ...
            raise AuthInvalid(
                ...
                f"Run `claude-migrate add {name}` to recreate it."
            ) from e
    try:
        return Profile(**data)
        ...
        try:
            ...
            if attempt == MAX_PASTE_RETRIES:
                msg = (
                    ...
                    "or `claude-migrate add <profile>` to retry."
                )
                if extra_help:
                    msg += f"\n\n{extra_help}"
...
async def run_auth_flow(profile_name: str, *, refreshing: bool = False) -> Profile:
    ...
    print(f"  → `claude-migrate add {profile_name}`      re-paste cookies after expiry")
    return profile
